...
const getProducts = async()=>{
  const response = await client.fetch(`*[_type=='product' && category -> title == 'Female']{_id,title,description,price,image,item,category -> {title}}`);
  return response;
}
...
export default async function page() {
  ...
  return (
    <div className=" flex flex-row gap-[85px] mt-20 mb-16">
          {productsData.length > 0 ? 
          (productsData.map((item)=>(
            <Link href={`/products/${item.title}`}>
             <div key={item._id}>
                <Image src={urlFor(item.image).url()} width='280' height='280' alt="products" />
